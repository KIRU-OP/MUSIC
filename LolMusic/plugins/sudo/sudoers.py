...
from LolMusic.utils.database import add_sudo, remove_sudo
from LolMusic.utils.decorators.language import language
from LolMusic.utils.extraction import extract_user
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("ᴋᴀɴʜᴀ ᴋᴀ 𝗟ᴜɴᴅ 𝗟ᴇɢᴀ 😂 \n 𝗦ᴜᴅᴏʟɪsᴛ 𝗗ᴇᴋʜɴᴇ ᴀᴀʏᴀ ᴍᴄ 🤧🖕", show_alert=True)
    else:
    ...
    for user_id in SUDOERS.copy():
        if user_id != OWNER_ID:
            ...
            if removed:
                ...
                count -= 1
    await message.reply_text(f"**ʀᴇᴍᴏᴠᴇᴅ {count} ᴜsᴇʀs ғʀᴏᴍ sᴜᴅᴏ ʟɪsᴛ.**")
